...
class IntelligenceToNav2(Node):
        ...
        self.declare_parameter(
            ...
            descriptor=ParameterDescriptor(
                description='Absolute path to the semantic map YAML. '
                            'If empty, defaults to <pkg_share>/config/rrc_map_semantics.yaml'
            )
        )
        self.declare_parameter(
    ...
    def _load_locations(self) -> dict:
        ...
        if not yaml_path:
            # Default: look inside the installed package share
            try:
                pkg_share = get_package_share_directory('wheelchair_intelligence_addons')
                yaml_path = os.path.join(pkg_share, 'config', 'rrc_map_semantics.yaml')
            except Exception:
                self.get_logger().error(
                    'Could not locate package share directory. '
                    'Set the semantics_yaml parameter explicitly.'
                )
                return {}
    ...
    def _publish_status(self):
        """Broadcast current navigation state on /nav_status."""
        status_msg = String()
